...
exports.labReportSchema = z.object({
  labName:                        z.string().min(1, "Lab name is required"),
  dateLabReceived:                z.string().min(1, "Date lab received is required").refine(d => !isNaN(new Date(d).getTime()), "Invalid date lab received"),
  ...
  dateLabSentDistrict:            z.string().min(1, "Date lab sent to district is required").refine(d => !isNaN(new Date(d).getTime()), "Invalid date lab sent to district"),
  dateDistrictReceivedLabResult:  z.string().min(1, "Date district received result is required").refine(d => !isNaN(new Date(d).getTime()), "Invalid date district received result"),
