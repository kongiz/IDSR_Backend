...
const toInt = z.union([z.string(), z.number()]).transform(Number).refine(Number.isInteger, "Must be an integer");
const toNum = (def = 0) => z.union([z.string(), z.number()]).optional().default(def).transform(Number);

exports.surveillanceReportSchema = z.object({
  ...
  dateFrom:     z.string().min(1, "Date from is required").refine(d => !isNaN(new Date(d).getTime()), "Invalid date from"),
  dateTo:       z.string().min(1, "Date to is required").refine(d => !isNaN(new Date(d).getTime()), "Invalid date to"),
